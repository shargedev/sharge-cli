...
export function requireNotesApiKey(
...
function remainingNotesTimeout(
  runtime: CliRuntime,
  options: NotesRequestContext,
  cause?: CliFailure,
): number {
  ...
  if (remaining <= 0) {
    throw new CliFailure({
      ...
      message: "Notes 命令在完成权限恢复前已超过总超时。",
...
export async function recoverNotesPermission(
  runtime: CliRuntime,
  options: NotesRequestContext,
  error: CliFailure,
): Promise<never> {
  ...
  const status = await requestJson<unknown>(runtime, {
    ...
    timeoutMs: remainingNotesTimeout(runtime, options, error),
  });
  const currentScopes =
...
export async function requestNotes<T>(
  runtime: CliRuntime,
  options: NotesRequestContext & {
    path: string;
    body?: unknown;
  ...
  try {
    return await requestJson<T>(runtime, {
      ...
      path: options.path,
      timeoutMs: remainingNotesTimeout(runtime, options),
    ...
    return await recoverNotesPermission(runtime, options, error);
  }
}
